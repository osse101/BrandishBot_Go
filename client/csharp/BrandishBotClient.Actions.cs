...
namespace BrandishBot.Client
{
    public partial class BrandishBotClient
    {
        ...
        {
            var query = BuildQuery("platform=" + Uri.EscapeDataString(platform), "platform_id=" + Uri.EscapeDataString(platformId), "user=" + Uri.EscapeDataString(username));
        ...
        {
            var query = BuildQuery("platform=" + Uri.EscapeDataString(platform), "user=" + Uri.EscapeDataString(username));
            var response = await GetAsync<RecipeListResponse>("/api/v1/recipes" + query);
            return response?.Recipes ?? new List<Recipe>();
        }
    }
