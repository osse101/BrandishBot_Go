...
import type { User, InventoryItem, UserJob, EventLogEntry, QuestProgress } from '../api/types';
...
export function UsersPage() {
  ...
  const [activeTab, setActiveTab] = useState<'inventory' | 'jobs' | 'stats' | 'quests' | 'events'>('inventory');
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setUser(null);
    try {
      const u = await apiGet<User>(`/api/v1/admin/user/lookup?platform=${platform}&username=${encodeURIComponent(username.trim())}`);
      setUser(u);
  ...
  const loadTabData = async (tab: string, plat: string, uname: string) => {
    try {
      switch (tab) {
        case 'inventory': {
          const inv = await apiGet<{ inventory: InventoryItem[] }>(`/api/v1/user/inventory-by-username?platform=${plat}&username=${encodeURIComponent(uname)}`);
          setInventory(inv.inventory ?? []);
          break;
        }
        case 'jobs': {
          const j = await apiGet<{ jobs: UserJob[] }>(`/api/v1/jobs/user?platform=${plat}&username=${encodeURIComponent(uname)}`);
          setJobs(j.jobs ?? []);
          break;
        }
        case 'stats': {
          const s = await apiGet<unknown>(`/api/v1/stats/user?platform=${plat}&username=${encodeURIComponent(uname)}`);
          setStats(s);
          break;
        }
        case 'quests': {
          const q = await apiGet<{ quests: QuestProgress[] }>(`/api/v1/quests/progress?platform=${plat}&username=${encodeURIComponent(uname)}`);
          setQuests(q.quests ?? []);
          break;
        }
        case 'events': {
          if (user) {
            const ev = await apiGet<{ events: EventLogEntry[] }>(`/api/v1/admin/events?user_id=${user.id}&limit=50`);
            setEvents(ev.events ?? []);
          }
          break;
        }
  ...
  // Admin actions
  const [addItemForm, setAddItemForm] = useState({ item_name: '', quantity: '1' });
  const [removeItemForm, setRemoveItemForm] = useState({ item_name: '', quantity: '1' });
      ...
      {user && (
        <div className="bg-gray-900 rounded-lg border border-gray-800">
          <div className="p-4 border-b border-gray-800">
            <div className="flex items-center gap-4">
              <div>
                ...
                <p className="text-xs text-gray-500">{user.platform} | ID: {user.id}</p>
              </div>
            </div>
          </div>
          ...
          <div className="p-4">
            ...
            {activeTab === 'stats' && (
              stats ? <JsonViewer data={stats} defaultExpanded /> : <p className="text-sm text-gray-500">No stats</p>
            )}
            ...
            {activeTab === 'quests' && (
              <DataTable
                columns={[
                  ...
                  { key: 'completed', header: 'Done', render: (r) => (r as Record<string, unknown>).completed ? 'Yes' : 'No' },
                ]}
              ...
              <div className="space-y-1 max-h-96 overflow-y-auto">
                ...
                {events.map(evt => (
                  <div key={evt.id} className="text-xs border-b border-gray-800 py-2">
                    <span className="text-gray-500 font-mono mr-2">{new Date(evt.created_at).toLocaleString()}</span>
      ...
      {user && (
        <div className="bg-gray-900 rounded-lg border border-gray-800 p-4">
          ...
          >
            ...
            <button type="submit" className="px-3 py-1.5 text-sm rounded-md bg-green-600 text-white hover:bg-green-500 transition-colors">Add</button>
          </form>
          ...
          >
            <div className="flex-1">
              ...
              <input
                ...
                onChange={e => setRemoveItemForm(f => ({ ...f, item_name: e.target.value }))}
                className="w-full px-2 py-1.5 bg-gray-800 border border-gray-700 rounded text-sm text-gray-200 focus:outline-none focus:border-blue-500"
                placeholder="item name"
              />
            </div>
            ...
            <button type="submit" className="px-3 py-1.5 text-sm rounded-md bg-red-600 text-white hover:bg-red-500 transition-colors">Remove</button>
          </form>
          ...
          >
            ...
            <button type="submit" className="px-3 py-1.5 text-sm rounded-md bg-blue-600 text-white hover:bg-blue-500 transition-colors">Award XP</button>
          </form>
          ...
          <button
            onClick={async () => {
              try {
                await apiPost('/api/v1/admin/timeout/clear', { platform, username: username.trim() });
