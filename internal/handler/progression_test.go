...
func TestProgressionHandlers_HandleVote(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Success",
			body: VoteRequest{UserID: "u1", NodeKey: "n1"},
			setupMock: func(m *mocks.MockProgressionService) {
				m.On("VoteForUnlock", mock.Anything, "u1", "n1").Return(nil)
			},
		...
		},
		{
			...
			body:           VoteRequest{UserID: "", NodeKey: "n1"},
			setupMock:      func(m *mocks.MockProgressionService) {},
		...
		},
		{
			...
			body: VoteRequest{UserID: "u1", NodeKey: "n1"},
			setupMock: func(m *mocks.MockProgressionService) {
				m.On("VoteForUnlock", mock.Anything, "u1", "n1").Return(errors.New("already voted"))
			},
