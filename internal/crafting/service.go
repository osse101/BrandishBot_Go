...
func (s *service) UpgradeItem(ctx context.Context, platform, platformID, username, itemName string, quantity int) (*CraftingResult, error) {
	...
	// Award Blacksmith XP (synchronous for feedback)
	xpResult, err := s.awardBlacksmithXP(ctx, user.ID, actualQuantity, "upgrade", itemName)
	if err != nil {
		log.Warn("Failed to award Blacksmith XP", "error", err)
	}
...
func (s *service) DisassembleItem(ctx context.Context, platform, platformID, username, itemName string, quantity int) (*DisassembleResult, error) {
	...
	// Award Blacksmith XP (synchronous for feedback)
	xpResult, err := s.awardBlacksmithXP(ctx, user.ID, actualQuantity, "disassemble", itemName)
	if err != nil {
		log.Warn("Failed to award Blacksmith XP", "error", err)
	}
