...
func TestInventoryCommand_Grouping(t *testing.T) {
	ctx := SetupTestContext(t)
	cmd, handler := InventoryCommand()
	...
	ctx.Mux.HandleFunc("/api/v1/user/inventory", func(w http.ResponseWriter, r *http.Request) {
		WriteJSON(w, map[string]interface{}{
			"items": []user.InventoryItem{
				{PublicName: "sword", Quantity: 1},
				{PublicName: "potion", Quantity: 5},
				{PublicName: "sword", Quantity: 2}, // Same name, should be grouped
			},
		})
	})
	...
	assert.NotNil(t, sentEmbed)
	if sentEmbed != nil {
		assert.Contains(t, sentEmbed.Description, "**sword** x3") // 1 + 2
		assert.Contains(t, sentEmbed.Description, "**potion** x5")
		// Ensure "sword" doesn't appear twice (though Contains doesn't strictly check for count, we can check for the exact string)
		assert.NotContains(t, sentEmbed.Description, "**sword** x1")
		assert.NotContains(t, sentEmbed.Description, "**sword** x2")
	}
}
